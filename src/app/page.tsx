...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden" style={{ background: "linear-gradient(135deg, #4F5B35 0%, #6B7A45 40%, #5a6840 70%, #3d4728 100%)" }}>
      <div className="pointer-events-none absolute inset-0 opacity-30" style={{ background: "radial-gradient(ellipse 80% 60% at 60% 20%, #C9A96A33 0%, transparent 70%)" }} />
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20">
        ...
        <section className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
          <div className="flex flex-col gap-7">
            ...
            <motion.h1 {...anim(0.1)} className="text-5xl font-bold leading-[1.12] tracking-tight sm:text-6xl" style={{ color: "#F8F6F1" }}>
              Financial Management,{" "}
              <span style={{ color: "#C9A96A" }}>Made Simple</span>
            </motion.h1>
            <motion.p {...anim(0.2)} className="max-w-lg text-base leading-8" style={{ color: "rgba(248,246,241,0.75)" }}>
            ...
            <motion.div {...anim(0.3)} className="flex flex-wrap gap-4">
              ...
              <Link href="/portal" className="rounded-full border px-7 py-3.5 text-sm font-semibold transition-all duration-300 hover:scale-105" style={{ borderColor: "rgba(248,246,241,0.2)", background: "rgba(248,246,241,0.06)", color: "#F8F6F1", backdropFilter: "blur(8px)" }}>
          ...
          <motion.div
            ...
            style={{ background: "rgba(248,246,241,0.07)", backdropFilter: "blur(20px)", borderColor: "rgba(248,246,241,0.12)", boxShadow: "0 24px 60px rgba(0,0,0,0.25), inset 0 1px 0 rgba(248,246,241,0.1)" }}
          >
            ...
            <div className="space-y-4">
              {portalFeatures.map((item, i) => (
                <motion.div key={item.title} {...anim(0.3 + i * 0.1)} className="flex items-start gap-4 rounded-2xl p-4 transition-all duration-300 hover:scale-[1.02]" style={{ background: "rgba(248,246,241,0.06)", border: "1px solid rgba(248,246,241,0.08)" }}>
        ...
        <section className="mt-24">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {services.map((item, i) => (
              <motion.div key={item.title} {...inView(i * 0.08)} className="rounded-[24px] p-6 transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)]" style={{ background: "rgba(248,246,241,0.06)", backdropFilter: "blur(12px)", border: "1px solid rgba(248,246,241,0.09)" }}>
                ...
                <p className="text-sm font-semibold" style={{ color: "#F8F6F1" }}>{item.title}</p>
                <p className="mt-2 text-sm leading-7" style={{ color: "rgba(248,246,241,0.5)" }}>{item.description}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        {/* How it works */}
        <section className="mt-24">
          <motion.div {...inView(0)} className="rounded-[28px] p-10" style={{ background: "rgba(248,246,241,0.05)", backdropFilter: "blur(16px)", border: "1px solid rgba(248,246,241,0.08)" }}>
            <p className="mb-8 text-xs font-semibold uppercase tracking-[0.3em]" style={{ color: "#C9A96A" }}>How It Works</p>
            <div className="grid gap-6 sm:grid-cols-3">
              {steps.map((step, i) => (
                <motion.div key={step} {...inView(i * 0.1)} className="rounded-2xl p-6" style={{ background: "rgba(248,246,241,0.05)", border: "1px solid rgba(248,246,241,0.07)" }}>
        ...
        <section className="mt-16 grid gap-5 lg:grid-cols-3">
          {trust.map((item, i) => (
            <motion.div key={item.title} {...inView(i * 0.1)} className="rounded-[24px] p-6 transition-all duration-300 hover:scale-[1.02]" style={{ background: "rgba(248,246,241,0.05)", backdropFilter: "blur(10px)", border: "1px solid rgba(248,246,241,0.08)" }}>
              <p className="text-sm font-semibold" style={{ color: "#C9A96A" }}>{item.title}</p>
              <p className="mt-3 text-sm leading-7" style={{ color: "rgba(248,246,241,0.5)" }}>{item.body}</p>
            </motion.div>
          ))}
        </section>
      </div>
